...
//        }
        ...
        if (std::chrono::duration<double>(time_2 - time_1).count() > 0.05)
        {
            std::cerr << "Failed in A star path searching !!! 0.2 seconds time limit exceeded." << std::endl;
            return false;
        }
    }
    ...
    if (std::chrono::duration<double>(time_2 - time_1).count() > 0.1)
        std::cerr << "Time consume in A star path finding is "
                  << std::chrono::duration<double>(time_2 - time_1).count()
                  << "s, iter=" << num_iter << std::endl;
