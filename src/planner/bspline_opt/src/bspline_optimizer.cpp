...
static const rclcpp::Logger BSPLINE_OPT_LOGGER = rclcpp::get_logger("bspline_opt");
#define ROS_ERROR(...) RCLCPP_ERROR(BSPLINE_OPT_LOGGER, __VA_ARGS__)
#define ROS_WARN(...) RCLCPP_WARN(BSPLINE_OPT_LOGGER, __VA_ARGS__)
...
namespace ego_planner
{
    ...
    void BsplineOptimizer::setParam(const rclcpp::Node::SharedPtr &nh)
    {
        ...
        param("optimization/lambda_smooth", lambda1_, -1.0);
        param("optimization/lambda_collision", lambda2_, -1.0);
        param("optimization/lambda_feasibility", lambda3_, -1.0);
        param("optimization/lambda_fitness", lambda4_, -1.0);

        param("optimization/dist0", dist0_, -1.0);
        param("optimization/max_vel", max_vel_, -1.0);
        param("optimization/max_acc", max_acc_, -1.0);

        param("optimization/order", order_, 3);
    }
