...
public:
    ...
    PatchWork(const rclcpp::Node::SharedPtr &nh) : node_handle_(nh) {
        // Init ROS related
        RCLCPP_INFO(node_handle_->get_logger(), "Inititalizing PatchWork...");

        node_handle_->declare_parameter<double>("sensor_height", 1.723);
        node_handle_->get_parameter("sensor_height", sensor_height_);
        node_handle_->declare_parameter<bool>("patchwork.verbose", false);
        node_handle_->get_parameter("patchwork.verbose", verbose_);

        node_handle_->declare_parameter<int>("patchwork.num_iter", 3);
        node_handle_->get_parameter("patchwork.num_iter", num_iter_);
        node_handle_->declare_parameter<int>("patchwork.num_lpr", 20);
        node_handle_->get_parameter("patchwork.num_lpr", num_lpr_);
        node_handle_->declare_parameter<int>("patchwork.num_min_pts", 10);
        node_handle_->get_parameter("patchwork.num_min_pts", num_min_pts_);
        node_handle_->declare_parameter<double>("patchwork.th_seeds", 0.4);
        node_handle_->get_parameter("patchwork.th_seeds", th_seeds_);
        node_handle_->declare_parameter<double>("patchwork.th_dist", 0.3);
        node_handle_->get_parameter("patchwork.th_dist", th_dist_);
        node_handle_->declare_parameter<double>("patchwork.max_r", 80.0);
        node_handle_->get_parameter("patchwork.max_r", max_range_);
        node_handle_->declare_parameter<double>("patchwork.min_r", 2.7);
        node_handle_->get_parameter("patchwork.min_r", min_range_); // It indicates bodysize of the car.
        node_handle_->declare_parameter<int>("patchwork.uniform.num_rings", 30);
        node_handle_->get_parameter("patchwork.uniform.num_rings", num_rings_);
        node_handle_->declare_parameter<int>("patchwork.uniform.num_sectors", 108);
        node_handle_->get_parameter("patchwork.uniform.num_sectors", num_sectors_);
        node_handle_->declare_parameter<double>("patchwork.uprightness_thr", 0.5);
        node_handle_->get_parameter("patchwork.uprightness_thr", uprightness_thr_); // The larger, the more strict
        node_handle_->declare_parameter<double>("patchwork.adaptive_seed_selection_margin", -1.1);
        node_handle_->get_parameter("patchwork.adaptive_seed_selection_margin", adaptive_seed_selection_margin_); // The more larger, the more soft
        ...
        node_handle_->declare_parameter<bool>("patchwork.using_global_elevation", true);
        node_handle_->get_parameter("patchwork.using_global_elevation", using_global_thr_);
        node_handle_->declare_parameter<double>("patchwork.global_elevation_threshold", 0.0);
        node_handle_->get_parameter("patchwork.global_elevation_threshold", global_elevation_thr_);
        ...
        RCLCPP_INFO(node_handle_->get_logger(), "Num of LPR: %d", num_lpr_);
        RCLCPP_INFO(node_handle_->get_logger(), "Num of min. points: %d", num_min_pts_);
        RCLCPP_INFO(node_handle_->get_logger(), "Seeds Threshold: %f", th_seeds_);
        RCLCPP_INFO(node_handle_->get_logger(), "Distance Threshold: %f", th_dist_);
        RCLCPP_INFO(node_handle_->get_logger(), "Max. range:: %f", max_range_);
        RCLCPP_INFO(node_handle_->get_logger(), "Min. range:: %f", min_range_);
        RCLCPP_INFO(node_handle_->get_logger(), "Num. rings: %d", num_rings_);
        RCLCPP_INFO(node_handle_->get_logger(), "Num. sectors: %d", num_sectors_);
        RCLCPP_INFO(node_handle_->get_logger(), "adaptive_seed_selection_margin: %f", adaptive_seed_selection_margin_);

        // CZM denotes 'Concentric Zone Model'. Please refer to our paper
        node_handle_->declare_parameter<int>("patchwork.czm.num_zones", 4);
        node_handle_->get_parameter("patchwork.czm.num_zones", num_zones_);
        node_handle_->declare_parameter<std::vector<int64_t>>("patchwork.czm.num_sectors_each_zone", std::vector<int64_t>{16, 32, 54, 32});
        node_handle_->get_parameter("patchwork.czm.num_sectors_each_zone", num_sectors_each_zone_);
        node_handle_->declare_parameter<std::vector<int64_t>>("patchwork.czm.num_rings_each_zone", std::vector<int64_t>{2, 4, 4, 4});
        node_handle_->get_parameter("patchwork.czm.num_rings_each_zone", num_rings_each_zone_);
        node_handle_->declare_parameter<std::vector<double>>("patchwork.czm.min_ranges_each_zone", std::vector<double>{2.7, 12.3625, 22.025, 41.35});
        node_handle_->get_parameter("patchwork.czm.min_ranges_each_zone", min_ranges_);
        node_handle_->declare_parameter<std::vector<double>>("patchwork.czm.elevation_thresholds", std::vector<double>{0.0, 0.0, 0.0, 0.0});
        node_handle_->get_parameter("patchwork.czm.elevation_thresholds", elevation_thr_);
        node_handle_->declare_parameter<std::vector<double>>("patchwork.czm.flatness_thresholds", std::vector<double>{0.0, 0.0, 0.0, 0.0});
        node_handle_->get_parameter("patchwork.czm.flatness_thresholds", flatness_thr_);


        RCLCPP_INFO(node_handle_->get_logger(), "\033[1;32mUprightness\33[0m threshold: %f", uprightness_thr_);
        RCLCPP_INFO(node_handle_->get_logger(), "\033[1;32mElevation\33[0m thresholds: %f %f %f %f", elevation_thr_[0],elevation_thr_[1], elevation_thr_[2], elevation_thr_[3]);
        RCLCPP_INFO(node_handle_->get_logger(), "\033[1;32mFlatness\033[0m thresholds: %f %f %f %f", flatness_thr_[0], flatness_thr_[1], flatness_thr_[2], flatness_thr_[3]);
...
//        cout_params();
        ...
        node_handle_->declare_parameter<bool>("patchwork.visualize", true);
        node_handle_->get_parameter("patchwork.visualize", visualize_);
        ...
        revert_pc_pub = node_handle_->create_publisher<sensor_msgs::msg::PointCloud2>("/revert_pc", 100);
        reject_pc_pub = node_handle_->create_publisher<sensor_msgs::msg::PointCloud2>("/reject_pc", 100);
...
private:
    ...
    PatchworkPolygonArray poly_list_;

    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr revert_pc_pub;
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr reject_pc_pub;
     ...
     */
    geometry_msgs::msg::PolygonStamped set_polygons(int r_idx, int theta_idx, int num_split);
        ...
        for (uint16_t ring_idx = 0; ring_idx < num_rings_each_zone_[k]; ++ring_idx) {
            for (uint16_t sector_idx = 0; sector_idx < num_sectors_each_zone_[k]; ++sector_idx) {
                if (zone[ring_idx][sector_idx].points.size() > num_min_pts_) {
                    ...
                    double t_tmp2 = node_handle_->now().seconds();
                    ...
                    double t_tmp3 = node_handle_->now().seconds();
...
//    time_txt.close();
    ...
    // jsk_recognition_msgs is unavailable in this environment; keep algorithm/output unchanged.
}
    ...
    for (auto const &pt : src.points) {
        ...
        if ((r <= max_range_) && (r > min_range_)) {
            ...
            if (r < min_range_z2_) { // In First rings
                ring_idx   = min(static_cast<int>(((r - min_range_) / ring_sizes_[0])), static_cast<int>(num_rings_each_zone_[0]) - 1);
                sector_idx = min(static_cast<int>((theta / sector_sizes_[0])), static_cast<int>(num_sectors_each_zone_[0]) - 1);
                czm[0][ring_idx][sector_idx].points.emplace_back(pt);
            } else if (r < min_range_z3_) {
                ring_idx   = min(static_cast<int>(((r - min_range_z2_) / ring_sizes_[1])), static_cast<int>(num_rings_each_zone_[1]) - 1);
                sector_idx = min(static_cast<int>((theta / sector_sizes_[1])), static_cast<int>(num_sectors_each_zone_[1]) - 1);
                czm[1][ring_idx][sector_idx].points.emplace_back(pt);
            } else if (r < min_range_z4_) {
                ring_idx   = min(static_cast<int>(((r - min_range_z3_) / ring_sizes_[2])), static_cast<int>(num_rings_each_zone_[2]) - 1);
                sector_idx = min(static_cast<int>((theta / sector_sizes_[2])), static_cast<int>(num_sectors_each_zone_[2]) - 1);
                czm[2][ring_idx][sector_idx].points.emplace_back(pt);
            } else { // Far!
                ring_idx   = min(static_cast<int>(((r - min_range_z4_) / ring_sizes_[3])), static_cast<int>(num_rings_each_zone_[3]) - 1);
                sector_idx = min(static_cast<int>((theta / sector_sizes_[3])), static_cast<int>(num_sectors_each_zone_[3]) - 1);
...
template<typename PointT>
inline
geometry_msgs::msg::PolygonStamped PatchWork<PointT>::set_polygons(int r_idx, int theta_idx, int num_split) {
...
template<typename PointT>
inline
geometry_msgs::msg::PolygonStamped PatchWork<PointT>::set_polygons(int zone_idx, int r_idx, int theta_idx, int num_split) {
    geometry_msgs::msg::PolygonStamped polygons;
    // Set point of polygon. Start from RL and ccw
    geometry_msgs::msg::Point32        point;
